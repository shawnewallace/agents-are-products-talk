...
- Not yet done (see `punch-list.md`): final timing pass against the 52-min budget (now with ~3–4 min §6 slack), one real agent run against the Scenario 4 sample to replace the mocked output on "What the agent produced," verify X/LinkedIn handles, publish slides/checklist/agent files to the session page.
