...
- Not yet done (see `punch-list.md`): final timing pass against the 52-min budget (§5 now ~1 min heavier — trim from §8 if running long, existing fallback covers it), one real agent run against the Scenario 4 sample to replace the mocked output on "What the agent produced," verify X/LinkedIn handles, publish slides/checklist/agent files to the session page.
